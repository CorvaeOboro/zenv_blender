...
# remove unused textures, materials, and mesh data
...
class ZENV_OT_CleanUnusedTextures(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for img in bpy.data.images[:]:  # Use slice to avoid modifying during iteration
...
class ZENV_OT_CleanMissingTextures(bpy.types.Operator):
    """Remove or fix missing texture references"""
    bl_idname = "zenv.clean_missing_textures"
    bl_label = "Clean Missing Textures"
    bl_description = "Remove references to missing texture files and attempt to find relocated textures"
    bl_options = {'REGISTER', 'UNDO'}
...
class ZENV_OT_CleanMeshData(bpy.types.Operator):
    ...
    bl_description = "Remove doubles, dissolve loose vertices, and recalculate normals"
    bl_options = {'REGISTER', 'UNDO'}
...
class ZENV_OT_RemoveEmptyVertexGroups(bpy.types.Operator):
    ...
    bl_description = "Remove all vertex groups that have no vertices assigned"
    bl_options = {'REGISTER', 'UNDO'}
