...
bl_info = {
    ...
    "group_order": 70,
    "addon_order": 20,
    "tags": ['vfx', 'cloud', 'volumetric', 'procedural', 'metaball'],
    "description_short": 'Creates volumetric clouds using sphere clusters and point clouds',
    "description_medium": 'Creates volumetric clouds using sphere clusters, Fibonacci surface sampling, KD-tree advection simulation, multi-octave noise deformation, and metaball-based mesh generation with procedural volumetric materials.',
    "description_long": """
    VFX Volumetric Cloud Generator
Creates volumetric clouds using sphere clusters and point clouds.
Uses advection and noise to simulate cloud movement and shape.
Supports three cloud types: Cumulus (dense, puffy), Stratus (layered, flat), and Cirrus (wispy).""",
    "location": 'View3D > ZENV',
    "image_overview": 'zenv_blender_VFX_cloud.png',
    "addon_image": 'zenv_blender_VFX_cloud.png',
    "warning": '',
    "doc_url": '',
}
...
#endregion
#region PROPS
class ZENV_PG_VolumetricCloud(PropertyGroup):
...
#endregion
#region OP
class ZENV_OT_VolumetricCloud(Operator):
    ...
    def create_cloud_material(self, props):
        ...
        links.new(node_transparent.outputs['BSDF'], node_mix_shader.inputs[2])
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    pass
                logger.warning("Smoothing operation failed: %s", e)
            ...
            # Clean up metaball data (the object was converted to mesh)
            if meta.name in bpy.data.metaballs:
                bpy.data.metaballs.remove(meta)

    def execute(self, context):
        """Execute the volumetric cloud generation operation."""
        try:
            props = context.scene.zenv_cloud_props
            ...
            if props.output_type in {'POINTS', 'BOTH'}:
                ...
                created.append(point_obj.name)

            logger.info("Generated cloud: %s", ", ".join(created))
            self.report({'INFO'}, f"Generated cloud ({props.cloud_type.lower()})")
            return {'FINISHED'}

        except Exception as e:
            logger.exception("Cloud generation failed: %s", e)
...
#endregion
#region PANEL
class ZENV_PT_VolumetricCloud(Panel):
...
#region REG
classes = (
    ZENV_PG_VolumetricCloud,
    ZENV_OT_VolumetricCloud,
    ZENV_PT_VolumetricCloud,
)
...
def register():
    ...
    for current_class_to_register in classes:
        try:
            ...
            pass
    if not hasattr(bpy.types.Scene, "zenv_cloud_props"):
        bpy.types.Scene.zenv_cloud_props = PointerProperty(type=ZENV_PG_VolumetricCloud)
...
def unregister():
    ...
    for current_class_to_unregister in reversed(classes):
        try:
            ...
            pass
    if hasattr(bpy.types.Scene, "zenv_cloud_props"):
        delattr(bpy.types.Scene, "zenv_cloud_props")
    _uninstall_logger()
